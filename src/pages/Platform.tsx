      ...
      <PageContent title={PAGE_TITLE_PLATFORM} type="primary">
        <Box display="flex" flexDirection={xl ? 'column' : 'row'} gap={xl ? 2 : 6} alignItems="flex-start">
          <Box flexGrow={1} width={xl ? '100%' : 'auto'} display="flex" flexDirection="column">
            ...
            <Paragraph paragraph>
              No user can join without first being referred – and new users must link to at least one existing user via
              their INK ID at signup, guaranteeing 100% attribution. As such, the INK ID is both a user’s unique
              identifier, as well as their permanent referral code. Importantly, users may invite an unlimited number of
              users, expanding their earning potential.
            </Paragraph>
        ...
        >
          ...
          <Box flexGrow={1} width={xl ? '100%' : 'auto'} display="flex" flexDirection="column">
            ...
            <Paragraph paragraph>
              ...
              quickly share products and services with their unique INK IDs automatically embedded. We quickly turn web3
              projects, games, and other platform experiences into monetizable campaigns.
            </Paragraph>
            <Box display="flex" gap="24px" justifyContent="start" mt={2}>
              <RedButton>reserve ink</RedButton>
              <RedButton>read our litepaper</RedButton>
